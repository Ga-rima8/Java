...
.breadcrumb {
  ...
  color: var(--t3); letter-spacing: .4px;
}
...
.alert-close {
  ...
  opacity: .6; font-size: 11px; padding: 2px 4px;
}
...
.page-body {
  ...
  flex: 1;
  /* make sure content fits laptop without horizontal scroll */
  max-width: 100%;
  overflow-x: hidden;
}
...
.welcome-text h1 {
  font-family: 'Sora', sans-serif;
  font-size: 22px; font-weight: 700; color: var(--t1); margin-bottom: 5px;
}
...
.stat-card:hover {
  ...
  box-shadow: 0 8px 28px rgba(52,211,153,.1);
}
.stat-icon {
  width: 42px; height: 42px;
  border-radius: 11px;
...
.stat-bg-icon {
  ...
  font-size: 50px; opacity: .04; color: var(--g400); pointer-events: none;
}
...
.box-badge {
  ...
  background: rgba(52,211,153,.1); color: var(--g400);
...
.btn-ghost {
  ...
  color: var(--g400); background: rgba(52,211,153,.07);
...
.profile-hero {
  ...
  text-align: center;
  background: linear-gradient(to bottom, rgba(52,211,153,.04), transparent);
}
...
.avatar-ring {
  ...
  background: linear-gradient(135deg, var(--g400), var(--g700)) border-box;
...
.detail-row {
  display: flex; align-items: center; justify-content: space-between;
  padding: 8px 0;
  border-bottom: 1px solid rgba(52,211,153,.05);
}
...
.completion-fill {
  ...
  box-shadow: 0 0 7px rgba(52,211,153,.4);
}
...
.event-cat-badge {
  ...
  padding: 2px 6px; border-radius: 5px; margin-bottom: 6px;
}
.event-card h4 {
  font-family: 'Sora', sans-serif;
...
.cat-chip {
  ...
  padding: 9px 18px; transition: background .15s; cursor: pointer;
}
...
.cat-icon {
  ...
  display: grid; place-items: center;
  font-size: 12px; color: var(--g400); flex-shrink: 0;
  border: 1px solid rgba(52,211,153,.14);
}
...
@media (max-width: 1200px) {
  ...
  .content-grid { grid-template-columns: 270px 1fr; }
}
...
@media (max-width: 480px) {
  ...
  .box-header { padding: 13px 14px 11px; }
  .events-grid { padding: 10px; gap: 10px; }
}
</style>
</head>
<body>
      ...
      <div class="sb-user-avatar">
        <c:choose>
          <c:when test="${not empty user.profileImage}">
            <img src="${pageContext.request.contextPath}/uploads/${user.profileImage}" alt="avatar">
          </c:when>
        ...
        <div class="profile-hero">
          <div class="profile-avatar-wrap">
            <div class="profile-avatar">
              <c:choose>
                <c:when test="${not empty user.profileImage}">
                  <img src="${pageContext.request.contextPath}/uploads/${user.profileImage}" alt="profile">
                </c:when>
                <c:otherwise><span><c:out value="${not empty user.firstName ? user.firstName.substring(0,1).toUpperCase() : 'U'}"/></span></c:otherwise>
              </c:choose>
            </div>
      ...
      <div class="right-col">
              ...
              <div class="events-grid">
                <c:forEach var="ev" items="${recentEvents}">
                  <div class="event-card">
                    ...
                    <h4><c:out value="${ev.title}"/></h4>
                    <div class="event-meta">
                      <span><i class="fas fa-calendar"></i> <fmt:formatDate value="${ev.date}" pattern="dd MMM yyyy"/></span>
                      <span><i class="fas fa-map-marker-alt"></i> <c:out value="${ev.location}"/></span>
                      <span><i class="fas fa-user-tie"></i> <c:out value="${ev.host}"/></span>
                    </div>
                    <c:choose>
                      <c:when test="${ev.price == 0}"><span class="event-price free">Free</span></c:when>
                      <c:otherwise><span class="event-price">NPR <fmt:formatNumber value="${ev.price}" pattern="#,##0"/></span></c:otherwise>
                    </c:choose>
                  </div>
                </c:forEach>
              </div>
        ...
        <div class="box">
          <div class="box-header">
            ...
            <span class="box-badge"><c:out value="${not empty registeredCount ? registeredCount : 0}"/> events</span>
          </div>
          <c:choose>
                ...
                <table class="reg-table">
                  ...
                  <tbody>
                    <c:forEach var="ev" items="${registeredEvents}">
                      <tr>
                        <td><c:out value="${ev.title}"/></td>
                        <td><fmt:formatDate value="${ev.date}" pattern="dd MMM yyyy"/></td>
              ...
              <div class="cat-grid">
                <c:forEach var="cat" items="${categories}">
                  <div class="cat-chip">
                    ...
                    <div class="cat-info">
                      <strong><c:out value="${cat.title}"/></strong>
                      <small><c:out value="${cat.eventCount}"/> events · <c:out value="${cat.members}"/> members</small>
                    </div>
                  </div>
                </c:forEach>
              </div>
...
// Profile completion — reads live JSTL values
(function() {
  const fields = [
    ...
    '${not empty user.profileImage}'
  ];
  const filled = fields.filter(f => f === 'true').length;
...
setTimeout(function() {
  ...
  if (al) {
    ...
    al.style.opacity = '0';
    setTimeout(function(){ if(al && al.parentNode) al.remove(); }, 500);
  }
}, 4500);
</script>
</body>
